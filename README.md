...
- **Model operation shortcuts**: Quickly open the model selector, switch directly to Instant/Thinking/Pro modes, or cycle the current Thinking/Pro effort
...
3. Examples: `j`/`k` for scrolling, `Shift + Space` to toggle input focus, `Cmd/Ctrl + Enter` for safe send, `Cmd/Ctrl + Shift + 7/8/9` to switch to Instant/Thinking/Pro modes, `Cmd/Ctrl + E` to cycle Thinking/Pro effort, `Cmd/Ctrl + I` to open Temporary Chat, `Cmd/Ctrl + Shift + P` to pin/unpin the current chat, `Cmd/Ctrl + B` to branch the chat from the last assistant message.
